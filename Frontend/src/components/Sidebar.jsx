...
// If the inner width >= 768 sidebar will be seen alse it is a toggle button
  useEffect(() => {
    const handleResize = () => {
...
// If this sidebar is open and inner width < 768 then it will collapsw
    useEffect(() => {
...
// here this function will handle what tech is selected
...
const getQuestions = ()=>{
    if (!selectedLevel || !selectedTech) {
        return [];
    }
    return questionsData[selectedTech]?.[selectedLevel] || [];
}
//calcculate total score
...
const getPerformanceStatus = () => {
  ...
  const toggleSidebar = () => setIsSidebarOpen((prev) => !prev);// screen is less than 768 px
  const getAuthHeader = ()=>{
  ...
  const submitResult = async () => {
    ...
    if(!selectedLevel || !selectedTech) return;
        const payload = {
                        ...
                        <div key={tech.id} className={sidebarStyles.techItem} data-tech={tech.id}>
                          ...
                          >
                            ...
                            <div className={sidebarStyles.techButtonContent}>
                              <span className={`${sidebarStyles.techItem} ${tech.color}`}>
                                {tech.icon}
                              </span>
